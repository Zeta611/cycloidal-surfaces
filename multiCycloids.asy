...
path3 baseCurve = graph(base, tMin, 2pi + 1.8, operator ..);
draw(
    baseCurve,
    thin,
...
for (real angle = 0; angle < 2pi; angle += pi / 3) {
    path3 cycloidalCurve = graph(
        new triple (real t) { return paramCycloid(t, angle); },
        0, 2pi + 1.8, operator ..
    );
    if (angle == pi / 3) {
        draw(cycloidalCurve, blue + thick);
        label(
            "$\theta \equiv \frac\pi 3$",
            align=.5N + 3E,
            position=towardCamera(paramCycloid(pi, angle), distance=5)
        );
    } else if (angle == 2pi / 3) {
        draw(cycloidalCurve, blue + thick);
        label(
...
path3 cycloidalCurve = graph(
    new triple (real t) { return paramCycloid(t, t); },
    0, 2pi + 1.8, operator ..
);
draw(cycloidalCurve, red + thick, L=Label("$\theta(t) = t$"));

// Draw the cycloidal surface
nmesh = 24;
var cycloidalSurface = surface(paramCycloid, (0, 0), (2pi + 1.8, 2pi), Spline);
var surfacepen = material(
    white+opacity(.8),
    emissivepen=gray(.5)
);
var meshpen = black + linewidth(.1pt);
draw(cycloidalSurface, surfacepen=surfacepen, meshpen=meshpen);
